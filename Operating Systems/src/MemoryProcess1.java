...
public class MemoryProcess1 extends UserlandProcess{
	int amountToWrite = 3072; //Denotes the number of pages to write each time, in this case 3
	int timesRan = 0;
	/**
	 * Reads from and writes random bytes to memory along with the other 3 memory processes. Runs 15 times and then exits
	 */
	public void main() {
		while(true) {
			if(timesRan == 15) {
				System.out.println("MemoryProcess1 exiting and freeing memory");
				OS.exit();
			}			try {
				if(getTimesStarted() == 1) {
					...
					System.out.println("MemoryProcess1 read " + getArrayString(bytesRead));
					OS.AllocateMemory(amountToWrite);
				}
				else if(getTimesStarted()%4 == 2) {
					int freeingPageNumber = (int)OS.returnValue;
					int freeingIndex = (freeingPageNumber + 1) * 1024;
					...
					System.out.println("MemoryProcess1 wrote 3 pages to memory and is now freeing the page starting with " + 
				...
				else if(getTimesStarted()%4 == 3) OS.SwitchProcess();
				else OS.AllocateMemory(1024);
			} catch(Exception e) {
				e.printStackTrace();
			}
		}
