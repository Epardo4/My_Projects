...
public class MemoryProcess2 extends UserlandProcess{
	...
	public void main() {
		while(true) {
			if(timesRan == 15) {
				System.out.println("MemoryProcess2 exiting and freeing memory");
				OS.exit();
			}
			try {
				if(getTimesStarted()%4 == 1) OS.AllocateMemory(1024);
					...
					System.out.println("MemoryProcess2 read this from memory:" + getArrayString(bytesRead));
					...
					System.out.println("MemoryProcess2 wrote 3 pages to memory and is now freeing the page starting with " + 
