...
public class MemoryProcess4 extends UserlandProcess{
	...
	public void main() {
		while(true) {
			if(timesRan == 10) {
				System.out.println("MemoryProcess4 exiting and freeing memory");
				OS.exit();
			}			try {
				if(getTimesStarted()%4 == 1) OS.AllocateMemory(1024);
					...
					System.out.println("MemoryProcess4 read this from memory:" + getArrayString(bytesRead));
					...
					System.out.println("MemoryProcess4 wrote 5 pages to memory and is now freeing the page starting with " + 
